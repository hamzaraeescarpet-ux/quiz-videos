...
export default function Pricing() {
  ...
  const handleCheckout = (planType) => {
    ...
    // =========================================================================
    // DODO PAYMENTS CHECKOUT LINKS (Base URLs without redirect_url parameter)
    // =========================================================================
    // LIVE MODE LINKS (Used when you go live for real customers)
    const DODO_LIVE_MONTHLY_LINK = "https://checkout.dodopayments.com/buy/pdt_0NhJ8B12iV14QguVLXSkD";
    const DODO_LIVE_YEARLY_LINK = "https://checkout.dodopayments.com/buy/pdt_0NhJ9Hy2r0jliZi6RYOP3";

    // TEST MODE LINKS (Create these in Dodo Payments Dashboard under Test Mode)
    const DODO_TEST_MONTHLY_LINK = "https://test.checkout.dodopayments.com/buy/pdt_0NhaAGp32eu4mXeoM9cFw";
    const DODO_TEST_YEARLY_LINK = "https://test.checkout.dodopayments.com/buy/YOUR_TEST_YEARLY_ID";

    // -------------------------------------------------------------------------
    // TOGGLE TEST/LIVE MODE HERE:
    // Set to true to test with dummy cards on your live site.
    // Set to false when you want to accept real payments from customers.
    const USE_TEST_MODE = false; 
    // =========================================================================

    const checkoutBaseUrl = planType === 'yearly' 
      ? (USE_TEST_MODE ? DODO_TEST_YEARLY_LINK : DODO_LIVE_YEARLY_LINK)
      : (USE_TEST_MODE ? DODO_TEST_MONTHLY_LINK : DODO_LIVE_MONTHLY_LINK);

    if (checkoutBaseUrl.includes("YOUR_TEST_")) {
      alert("Test Mode links are not configured yet. Please toggle to Test Mode on your Dodo Payments Dashboard, create monthly/yearly test plans, and paste their links in Pricing.jsx.");
      return;
    }

    // Dynamically build redirect URL based on current host (so it works on localhost AND deployed site)
    const currentOrigin = window.location.origin; // e.g. "https://quizviral-nine.vercel.app" or "http://localhost:5173"
    const redirectUrl = encodeURIComponent(`${currentOrigin}/success`);
    
    // Construct final URL with redirect and email pre-fill
    const checkoutUrl = `${checkoutBaseUrl}?quantity=1&redirect_url=${redirectUrl}&email=${encodeURIComponent(currentUser.email)}`;
